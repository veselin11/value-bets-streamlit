...
st.divider() st.subheader("🎯 Value Bet Калкулатор")

with st.form("value_bet_form"): col1, col2 = st.columns(2) with col1: vb_match = st.text_input("Мач", key="vb_match") vb_odds = st.number_input("Коефициент от букмейкър", min_value=1.01, step=0.01, key="vb_odds") with col2: vb_probability = st.number_input("Твоя вероятност (%)", min_value=1.0, max_value=100.0, step=0.1, key="vb_prob")
