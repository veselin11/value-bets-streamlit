...
LIVESCORE_API_KEY = st.secrets["LIVESCORE_API_KEY"]
LIVESCORE_API_SECRET = st.secrets["LIVESCORE_API_SECRET"]
SPORT = "soccer"
...
@st.cache_data(ttl=3600)
def get_match_stats(match_id, api_key, api_secret):
    try:
        response = requests.get(
        ...
        st.error(f"LiveScore API Error: {e}")
        return None
...
def main():
    ...
    match = next((m for m in matches if selected_match == f"{m['bookmakers'][0]['markets'][0]['outcomes'][0]['name']} vs {m['bookmakers'][0]['markets'][0]['outcomes'][1]['name']}"), None)
