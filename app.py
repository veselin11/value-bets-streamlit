...
# --- Фиктивни данни ---
leagues = ["Английска Висша лига", "Испанска Ла Лига", "Италианска Серия А"]
matches_data = [
    {"league": random.choice(leagues), "home": f"Отбор {i}", "away": f"Съперник {i}", "odds": round(random.uniform(1.8, 3.5), 2), "value": round(random.uniform(5, 20), 1), "prob": round(random.uniform(40, 70), 1)}
    for i in range(1, 21)
]
...
if "target_profit" not in st.session_state:
    st.session_state.target_profit = 150.0
if "bet_history" not in st.session_state:
    st.session_state.bet_history = []

# --- Sidebar: Настройки ---
st.sidebar.title("Настройки")
st.session_state.bank = st.sidebar.number_input("Начална банка (лв)", value=st.session_state.bank, step=10.0)
st.session_state.target_profit = st.sidebar.number_input("Целева печалба (лв)", value=st.session_state.target_profit, step=10.0)
...
with tabs[0]:
    ...
    st.caption("Извличане на стойностни залози от API в реално време")
...
with tabs[1]:
    ...
    if st.session_state.bet_history:
        ...
        edited_df = st.data_editor(df_history, num_rows="dynamic", key="edit_history")
        st.session_state.bet_history = edited_df.to_dict("records")
    else:
...
with tabs[2]:
    ...
    history = st.session_state.bet_history
    if history:
        total_bets = len(history)
        won = sum(1 for b in history if b['result'] == 'Печели')
        lost = sum(1 for b in history if b['result'] == 'Губи')
        profit = sum((b['odds'] - 1) * b['stake'] if b['result'] == 'Печели' else -b['stake'] for b in history if b['result'] != '?')
        roi = (profit / sum(b['stake'] for b in history if b['result'] != '?')) * 100 if history else 0
        avg_value = sum(b['value'] for b in history) / total_bets
