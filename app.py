...
TARGET_PROFIT = st.sidebar.number_input("Целева печалба (лв)", value=150.0)
DAYS = st.sidebar.number_input("Брой дни", value=5)
START_BANKROLL = st.sidebar.number_input("Начална банка (лв)", value=500.0)
...
def calculate_bet_amount(odds):
    if odds <= 1.01:
        return 0
    raw_amount = daily_target / (odds - 1)
    return int(math.ceil(raw_amount / 10.0) * 10)  # Кръгло на 10
...
with tabs[2]:
    st.header("Настройки")
    st.write("Настройки за цел, банка и поддържани пазари.")
